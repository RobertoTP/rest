package com.usc.blog.web.rest;

import com.usc.blog.domain.Entry;
import com.usc.blog.repository.CommentRepository;
import com.usc.blog.repository.EntryRepository;
import com.usc.blog.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.usc.blog.domain.Entry}.
 */
@RestController
@RequestMapping("/api")
public class EntryResource {
    ...
    public EntryResource(EntryRepository entryRepository, CommentRepository commenntRepository) {
        this.entryRepository = entryRepository;
        this.commenntRepository = commenntRepository;
    }
     ...
     */
    @PostMapping(value = "/entries", produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    @PreAuthorize("hasRole('ROLE_ADMIN') or hasRole('ROLE_REDACTOR') or hasRole('ROLE_MODERATOR') or hasRole('ROLE_USER')")
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping(value = "/entries", produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    @PreAuthorize("hasRole('ROLE_ADMIN') or hasRole('ROLE_REDACTOR') or hasRole('ROLE_MODERATOR')")
     ...
     */
    @GetMapping(value = "/entries", produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
     ...
     */
    @GetMapping(value = "/entries/{id}", produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    public ResponseEntity<Entry> getEntry(@PathVariable Long id) {
        ...
        entry.get().setComments(commenntRepository.findByEntryId(id));
     ...
     */
    @DeleteMapping(value = "/entries/{id}", produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
